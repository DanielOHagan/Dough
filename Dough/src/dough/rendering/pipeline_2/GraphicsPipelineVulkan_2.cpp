...
namespace DOH {
	...
	void GraphicsPipelineInstanceInfo_2::enableOptionalFields() {
		if (mOptionalFields == nullptr) {
			mOptionalFields = std::make_unique<GraphicsPipelineOptionalFields_2>();
		}
	}

	GraphicsPipelineVulkan_2::GraphicsPipelineVulkan_2(GraphicsPipelineInstanceInfo_2& instanceInfo)
	:	mGraphicsPipeline(VK_NULL_HANDLE),
		mGraphicsPipelineLayout(VK_NULL_HANDLE),
		mInstanceInfo(instanceInfo)
	{}

	GraphicsPipelineVulkan_2::~GraphicsPipelineVulkan_2() {
	...
	void GraphicsPipelineVulkan_2::close(VkDevice logicDevice) {
	...
	void GraphicsPipelineVulkan_2::init(VkDevice logicDevice, VkExtent2D extent, VkRenderPass renderPass) {
		createPipelineLayout(logicDevice);
		createPipeline(logicDevice, extent, renderPass);
	}

	void GraphicsPipelineVulkan_2::recreate(VkDevice logicDevice, VkExtent2D extent, VkRenderPass renderPass) {
		vkDestroyPipeline(logicDevice, mGraphicsPipeline, nullptr);
		createPipeline(logicDevice, extent, renderPass);
	}

	void GraphicsPipelineVulkan_2::recordDrawCommands(uint32_t imageIndex, VkCommandBuffer cmd, CurrentBindingsState& currentBindings) {
		for (const auto& renderable : mRenderableDrawList) {

			//Bind descriptor sets
			std::shared_ptr<ShaderUniformSetsInstanceVulkan> shaderResourceData = renderable->getShaderResourceData();
			if (shaderResourceData != nullptr) {
				for (size_t i = 0; i < shaderResourceData->getDescriptorSets().size(); i++) {
			...
			for (const VkPushConstantRange& pushConstant : mInstanceInfo.getShaderProgram().getUniformLayout().getPushConstants()) {
				vkCmdPushConstants(
					cmd,
					mGraphicsPipelineLayout,
	...
	void GraphicsPipelineVulkan_2::createPipelineLayout(VkDevice logicDevice) {
		VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = {};
		pipelineLayoutCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
		pipelineLayoutCreateInfo.setLayoutCount = mInstanceInfo.getShaderProgram().getUniformLayout().getSetCount();
		std::vector<VkDescriptorSetLayout> layouts = mInstanceInfo.getShaderProgram().getUniformLayout().getNativeSetLayouts();
		pipelineLayoutCreateInfo.pSetLayouts = layouts.data();
		pipelineLayoutCreateInfo.pushConstantRangeCount = mInstanceInfo.getShaderProgram().getUniformLayout().getPushConstantCount();
		pipelineLayoutCreateInfo.pPushConstantRanges = mInstanceInfo.getShaderProgram().getUniformLayout().getPushConstants().data();
	...
	void GraphicsPipelineVulkan_2::createPipeline(VkDevice logicDevice, VkExtent2D extent, VkRenderPass renderPass) {
		...
		if (mInstanceInfo.hasOptionalFields()) {
			GraphicsPipelineOptionalFields_2& optFields = mInstanceInfo.getOptionalFields();
